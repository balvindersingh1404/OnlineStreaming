 ...
 */
public class InteractiveModeManager extends ModeManager implements IInteractiveMode {
    ...
    public InteractiveModeManager(int mode, MDGLHandler handler, Params params) {
        super(mode, handler);
        this.mStrategyType = "AbsInteractiveStrategy";
        mParams = params;
    ...
    protected AbsInteractiveStrategy createStrategyInteractive(int mode) {
        switch (mode){
    ...
    public void onOrientationChanged(final Activity activity) {
        getGLHandler().post(new Runnable() {
            @Override
            public void run() {
                getStrategyInteractive().onOrientationChanged(activity);
            }
        });
    }
    ...
    private class UpdateDragRunnable implements Runnable {
        ...
        @Override
        public void run() {
            getStrategyInteractive().handleDrag(distanceX, distanceY);
        }
    }
    ...
    public void onResume(Context context) {
        ...
        if (getStrategy().isSupport((Activity)context)){
            getStrategyInteractive().onResume(context);
        }
    }
    ...
    public void onPause(Context context) {
        mIsResumed = false;
        if (getStrategy().isSupport((Activity)context)){
            getStrategyInteractive().onPause(context);
        }
    }
}
